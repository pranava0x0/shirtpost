...
- **A resumable multi-step pipeline commits each side-effect's result as it lands and gates re-execution on it.** A failed run retried then RESUMES instead of repeating. The subtlety: persist the external id (posted-tweet id, charge id) in its *own* commit *before* the terminal state transition — if you set `id`, `status=done` together and the commit fails after the side-effect fired, the rollback loses the id and the retry re-fires it (double-post, double-charge). Truly closing the last gap needs an idempotency key on the external call; committing the id in its own step shrinks the window to near-zero. Regression-test that a retry with the id already set makes no external call.
- **A spend cap / rate guard must count on the column that signals the action, not a later terminal timestamp.** The external id (`x_tweet_id`) commits a step before the `published_at` that marks success. Counting the month's posts by `published_at` silently omits any drop that fired the side-effect then crashed before publishing (its `published_at` is NULL), so the cap is walked past. Count on the signal column itself, with a window that also catches the fired-but-unfinished rows — and bias the estimate toward over-counting (a guard should fail closed).
- **A poll/wait loop must log each attempt and surface the last status/error in its timeout.** `while not live: sleep()` that only checks the success case and raises a bare "never became ready" discards the cause — a permanently-broken target (auth disabled, wrong URL) is indistinguishable from a slow one, and the operator debugs blind. Capture the last status/exception, log each iteration, fast-fail on a status that will never heal (401/403), and put the last status + body excerpt in the raised error.
...
- **Two renderers of one artifact drift — share the geometry, don't duplicate it.** When the same output has a "source" form and a "real" form (an SVG debug source + the PNG that's actually printed, a preview + the export), a feature added to one silently diverges the other: add layout/case to the PNG path and the SVG still shows the old centered layout, so the "debuggable source" lies. Extract the shared decision (placement, scale, case) into one module both call; a per-renderer copy is a second source of truth that a diff won't flag. Regression-test that the two agree.
- **A per-item styling choice must flow to the thing actually produced, not just a derived preview.** Exposing "garment color" that flips the *ink* but not the ordered *variant* renders dark ink onto the default (black) blank → an invisible print. Before shipping a per-item option, trace it end-to-end to the external artifact/SKU; if it only changes a derived value, it's a broken half-feature — wire the real selector or don't expose it.
...
- **Never memoize an empty or failed result behind a retry affordance.** Caching `{quips: []}` (or a 5xx) for a trend and telling the user "try again" makes the button a no-op for the whole TTL — the retry keeps hitting the cached emptiness. Only cache a non-empty success; let failures and empties fall through so a retry actually re-runs. And prune the cache on write (evict expired keys) so an in-process `Map` keyed per-entity doesn't grow unbounded on a long-lived server.
- **A forward-only "ADD COLUMN" migration should trust the ALTER and swallow "duplicate column", not reflect-then-alter.** SQLite caches the schema *per connection*; a pooled connection that didn't see another connection's DDL reflects a stale column list, so `inspect(engine).get_columns()` can disagree with the `ALTER` that runs on a different connection (it bites test suites that churn DROP/CREATE). Attempting each `ALTER ... ADD COLUMN` in its own transaction and treating `duplicate column` as already-present is fully idempotent and immune to that staleness. Regression-test the *upgrade* path (old-schema DB → migrate → columns added, rows preserved as NULL, second run a no-op) — a fresh `create_all` never exercises the ALTER.
...
- **Pin exact versions, never floating ranges.** `==` (Python) + lockfile installs (`npm ci`, not `npm install`) — a `>=`/`^` range auto-pulls whatever the registry serves next, the exact window a bad release lands in. Better still, hash-lock (`pip-compile --generate-hashes` + `--require-hashes`, `uv lock`, lockfile integrity hashes) to reject same-version re-publishes. **Generate the hash-lock cross-platform** (`uv pip compile --universal`): a lock resolved only on your OS omits platform-conditional deps (e.g. `greenlet` under SQLAlchemy), and CI on the other OS then fails `--require-hashes` with "must be pinned upfront." A lock verified only on the author's machine is not verified for CI.
...
- **Validate an external numeric field on four axes: type, bool, finiteness, range.** `isinstance(x, int)` is `True` for `bool` (JSON `true` → a score of 1), and `json.loads` accepts `NaN`/`Infinity` by default — both slip past a naive `isinstance(x, (int, float))` and a `>= 0` check. For a field with a declared domain (e.g. a 0–100 judged score that later *bypasses* normalization and lands straight in a ranking), reject `bool`, reject non-finite (`math.isfinite`), and enforce the range at ingest — a `500` or `NaN` would otherwise corrupt the whole lane's scale. Regression-test each rejection.
...
- **Don't rank incomparable series on one scale — lane them, then normalize within the lane.** When a metric means different things across groups (a search-traffic estimate vs. a "present in feed" flag), a single global sort silently implies a comparison that doesn't exist. Group by the incomparable dimension, rank within each, and expose a within-group normalized value (min-max over that group's population, computed at read time) for any bar/meter. Carry the unit (`measurement`) so the record self-describes. **A laned UI must also *fetch* top-N per lane, not a global top-N** — a global `ORDER BY score LIMIT n` starves a low-magnitude lane out of existence (a 0–100 judged score never survives next to raw view counts in the thousands), so the lane renders empty even though rows exist. Use a per-partition `row_number()` (or one query per lane).
